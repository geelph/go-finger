...
//   - proxy: HTTP代理地址 (可为空)
//   - timeout: 超时时间(秒)
//   - workerCount: 指纹规则并发线程数，默认200（建议范围200-5000）
//
// 返回:
...
func FingerScan(target string, proxy string, timeout int, workerCount int) (*runner.TargetResult, error) {
	...
	if workerCount <= 0 {
		workerCount = runner.DefaultRuleWorkers // 使用默认规则工作池大小(200)
	}
...
func GetPoolStats() runner.GlobalRulePoolStats { return runner.GetPoolStats() }
...
// Defaults 描述库层暴露的关键默认配置，便于调用方自适应参数
type Defaults struct {
	URLWorkersDefault     int   // URL 并发默认值
	RuleWorkersDefault    int   // 规则并发默认值
	RuleWorkersMin        int   // 规则并发最小值
	RuleWorkersMax        int   // 规则并发最大值
	MaxResponseBodyBytes  int64 // 单次读取响应体大小上限
	NetworkDefaultTimeout int64 // 网络默认超时（秒）
}

// GetDefaults 返回当前库版本的关键默认配置
func GetDefaults() Defaults {
	return Defaults{
		URLWorkersDefault:     runner.DefaultURLWorkers,
		RuleWorkersDefault:    runner.DefaultRuleWorkers,
		RuleWorkersMin:        runner.MinRuleWorkers,
		RuleWorkersMax:        runner.MaxRuleWorkers,
		MaxResponseBodyBytes:  network.MaxDefaultBody,
		NetworkDefaultTimeout: int64(network.DefaultTimeout.Seconds()),
	}
}

// NormalizeURL 规范化目标地址（自动补全协议、按需探测 http/https）
// proxy 为空时不使用代理
func NormalizeURL(target, proxy string) (string, error) {
	return network.CheckProtocol(target, proxy)
}
