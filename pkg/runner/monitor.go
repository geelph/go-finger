...
// ForceReleaseMemory 强制释放系统内存
func ForceReleaseMemory() {
	before := GetMemoryStats()
	runtime.GC()
	debug.FreeOSMemory()
	after := GetMemoryStats()

	logger.Debug(fmt.Sprintf("强制释放内存完成，内存释放: %.2f MB",
		float64(before.HeapAlloc-after.HeapAlloc)/1024/1024))
}

// IsMemoryMonitorEnabled 检查内存监控是否启用
func IsMemoryMonitorEnabled() bool {
	return globalMonitor.enabled.Load()
}
